...
[~,NoOfColumn]= size(NUM); % may replace ~ for NoOfRows

for i = 2:NoOfColumn
    if (uint8(TXT{1,i}(3))>47 && uint8(TXT{1,i}(3))<58)
        TXT{1,i}=(uint8(TXT{1,i}(1))-48)*100 +(uint8(TXT{1,i}(2))-48)*10 +(uint8(TXT{1,i}(3))-48) ; %The number is bigger than 100
    else
        if (uint8(TXT{1,i}(2))>47 && uint8(TXT{1,i}(2))<58)
...
C=[double(LABEL);NUM];
C=sortrows(C',1,'MissingPlacement','first')';
